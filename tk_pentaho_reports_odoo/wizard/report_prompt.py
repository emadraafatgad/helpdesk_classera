...
#                                                                     ))
        ...
        setattr(cls, java_odoo.PARAM_XXX_2M_VALUE % counter, fields.Many2many("ir.actions.report.multivalues.promptwizard",
                                                                              ...
                                                                              string="Multi Select",
                                                                              _module='tk_pentaho_reports_odoo'
                                                                              ))
    return cls
...
class report_prompt_class(models.TransientModel):
    ...
    report_action_id = fields.Many2one('ir.actions.report', string='Report Name', readonly=True)
...
#                 mpwiz.write({'selected': True})
    ...
    def _find_report_action_from_context(self):
        report_action = self.env['ir.actions.report'].search([('report_name', '=', self.env.context.get('service_name', ''))], limit=1)
